...
class _LocationBottomSheetState extends State<LocationBottomSheet> {
  ...
  Future<void> _fetchData({String? parentId}) async {
    ...
    try {
      ...
      if (level == 0) {
        data = await _sb.from('lokasi').select('id_lokasi, nama_lokasi, unit(id_unit), is_star');
      } else if (level == 1) {
        if (_hasFullAccess) {
          data = await _sb.from('unit').select('id_unit, nama_unit, subunit(id_subunit), is_star').eq('id_lokasi', parentId!);
        } else if (widget.userUnitId != null) {
          data = await _sb.from('unit').select('id_unit, nama_unit, subunit(id_subunit), is_star')
              .eq('id_lokasi', parentId!).eq('id_unit', widget.userUnitId!);
        }
      } else if (level == 2) {
        data = await _sb.from('subunit').select('id_subunit, nama_subunit, area(id_area), is_star').eq('id_unit', parentId!);
      } else if (level == 3) {
        data = await _sb.from('area').select('id_area, nama_area, is_star').eq('id_subunit', parentId!);
      }
  ...
  void _onSearch(String query) {
    setState(() {
      ...
      _filteredData = _currentData.where((item) {
        return item[_getNameCol(_currentLevel)].toString().toLowerCase().contains(_searchQuery);
      }).toList();
      _sortData();
    });
  }
          ...
          Expanded(
            child: _isLoading
                ...
                : _filteredData.isEmpty
                    ? Center(child: Text(_bs('kosong'), style: const TextStyle(color: Colors.grey)))
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return GestureDetector(
                            ...
                            child: Container(
                              ...
                              padding: const EdgeInsets.all(12),
                              decoration: BoxDecoration(
                                color: const Color(0xFFF6FAFE), borderRadius: BorderRadius.circular(12),
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      // Camera button
                                      GestureDetector(
                                        onTap: () => _openCamera(item, itemId, itemName),
                                        child: Container(
                                          padding: const EdgeInsets.all(8),
                                          decoration: BoxDecoration(
                                            color: const Color(0xFF00C9E4).withOpacity(0.15),
