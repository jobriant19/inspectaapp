...
class KtsCameraWarmupService {
  KtsCameraWarmupService._();
  static final KtsCameraWarmupService instance = KtsCameraWarmupService._();
  ...
  Future<void> warmUp() async {
    ...
    try {
      ...
      debugPrint('Error warming up KTS camera: $e');
    } finally {
      isWarming = false;
    }
  }
...
class KtsCameraScreen extends StatefulWidget {
  final String lang;
  const KtsCameraScreen({super.key, required this.lang});
...
class _KtsCameraScreenState extends State<KtsCameraScreen> with WidgetsBindingObserver {
  ...
  Future<void> _initCamera() async {
    final warm = KtsCameraWarmupService.instance;
    ...
    try {
      ...
      debugPrint('Error initializing KTS camera: $e');
    }
  }
  ...
  Future<void> _setCamera(int index) async {
    ...
    try {
      ...
      debugPrint('Error setting KTS camera: ${e.code}\n${e.description}');
    }
  }
  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      debugPrint('Error taking KTS picture: ${e.code}\n${e.description}');
      if (mounted) {
  ...
  Future<void> _pickFromGallery() async {
    try {
      ...
      debugPrint('Error picking KTS image: $e');
    }
  }
        ...
        child: TweenAnimationBuilder<double>(
          ...
          child: Column(
            ...
            children: [
              ...
              _KtsPulsingText(text: _txt('preparing_camera'), color: _kAccent),
            ],
          ),
        ),
      ),
                      ...
                      Expanded(
                        child: Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              const Icon(CupertinoIcons.hammer_fill, color: _kAccent, size: 20),
...
class _KtsCameraIconButton extends StatelessWidget {
  final IconData icon;
  final VoidCallback onTap;
  final double size;

  const _KtsCameraIconButton({
    required this.icon,
    required this.onTap,
    this.size = 44,
  });
...
class _KtsFlashButton extends StatelessWidget {
  final bool supported;
  final bool enabled;
  final VoidCallback onTap;

  const _KtsFlashButton({
...
class _KtsPulsingText extends StatefulWidget {
  final String text;
  final Color color;

  const _KtsPulsingText({required this.text, required this.color});

  @override
  State<_KtsPulsingText> createState() => _KtsPulsingTextState();
}

class _KtsPulsingTextState extends State<_KtsPulsingText> with SingleTickerProviderStateMixin {
