...
class HelpCenterCameraWarmupService {
  HelpCenterCameraWarmupService._();
  static final HelpCenterCameraWarmupService instance =
      HelpCenterCameraWarmupService._();
  ...
  Future<void> warmUp() async {
    ...
    try {
      ...
      debugPrint('Error warming up Help Center camera: $e');
    } finally {
      isWarming = false;
    }
  }
...
class HelpCenterCameraScreen extends StatefulWidget {
  final String lang;

  const HelpCenterCameraScreen({
    super.key,
    this.lang = 'ID',
  });

  @override
  State<HelpCenterCameraScreen> createState() =>
      _HelpCenterCameraScreenState();
}

class _HelpCenterCameraScreenState extends State<HelpCenterCameraScreen>
  ...
  Future<void> _initCamera() async {
    final warm = HelpCenterCameraWarmupService.instance;
    ...
    try {
      ...
      debugPrint('Error initializing Help Center camera: $e');
    }
  }
  ...
  Future<void> _setCamera(int index) async {
    ...
    try {
      ...
      debugPrint(
          'Error setting Help Center camera: ${e.code}\n${e.description}');
    }
  }
  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      debugPrint(
          'Error taking Help Center picture: ${e.code}\n${e.description}');
      if (mounted) {
  ...
  Future<void> _pickFromGallery() async {
    try {
      ...
      debugPrint('Error picking Help Center image: $e');
    }
  }
        ...
        child: TweenAnimationBuilder<double>(
          ...
          child: Column(
            ...
            children: [
              ...
              _HelpCenterCameraPulsingText(text: _loadingText, color: _kAccent),
            ],
          ),
        ),
      ),
...
class _HelpCenterCameraIconButton extends StatelessWidget {
  final IconData icon;
  final VoidCallback onTap;
  final double size;

  const _HelpCenterCameraIconButton({
    required this.icon,
    required this.onTap,
    this.size = 44,
  });
...
class _HelpCenterCameraFlashButton extends StatelessWidget {
  final bool supported;
  final bool enabled;
  final VoidCallback onTap;

  const _HelpCenterCameraFlashButton({
...
class _HelpCenterCameraPulsingText extends StatefulWidget {
  final String text;
  final Color color;

  const _HelpCenterCameraPulsingText({required this.text, required this.color});

  @override
  State<_HelpCenterCameraPulsingText> createState() =>
      _HelpCenterCameraPulsingTextState();
}

class _HelpCenterCameraPulsingTextState
    extends State<_HelpCenterCameraPulsingText>
