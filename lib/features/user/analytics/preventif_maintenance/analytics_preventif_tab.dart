...
class AnalyticsPreventifTab extends StatefulWidget {
  final String lang;
  const AnalyticsPreventifTab({super.key, required this.lang});

  @override
  State<AnalyticsPreventifTab> createState() => _AnalyticsPreventifTabState();
}

class _AnalyticsPreventifTabState extends State<AnalyticsPreventifTab> {
  ...
  Future<void> _loadUserJabatan() async {
    ...
    try {
      ...
      debugPrint('AnalyticsPreventif loadUserJabatan error: $e');
    }
  }
  ...
  Future<void> _loadTableData() async {
    ...
    try {
      ...
      debugPrint('AnalyticsPreventif loadTableData error: $e');
  ...
  void _showKasieDetail(String kasieNama, String bagian) async {
    final months = _getMonths();
    final start  = months.first;
              ...
              Padding(
                ...
                child: Row(children: [
                  ...
                  Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(color: _PC.primaryLight, borderRadius: BorderRadius.circular(20)),
                    child: Text('${records.length} ${_t('records')}', style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w700, color: _PC.primary))),
                ]),
              ),
                              ...
                              child: Container(
                                ...
                                child: Row(children: [
                                  Container(width: 36, height: 36,
                                    decoration: BoxDecoration(color: _fileColor(r['file_name_pm']).withValues(alpha:0.12), borderRadius: BorderRadius.circular(8)),
                                    child: Icon(_fileIcon(r['file_name_pm']), color: _fileColor(r['file_name_pm']), size: 18)),
    ...
    } catch (e) { debugPrint('AnalyticsPreventif kasieDetail error: $e'); }
  }
  ...
  Widget _buildTable() {
    ...
    final bool canSeeDetail = _currentUserJabatan == 1;
                  ...
                  Expanded(flex: 6, child: GestureDetector(
                    ...
                    child: Text(row.kasieNama,
                      style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600,
                        color: row.total > 0 ? (canSeeDetail ? _PC.primary : _PC.textPrimary) : const Color(0xFFCBD5E1),
                        decoration: row.total > 0 && canSeeDetail ? TextDecoration.underline : TextDecoration.none),
                      overflow: TextOverflow.ellipsis))),
                ]),
              );
            }),
