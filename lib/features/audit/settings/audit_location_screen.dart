...
class _AuditLocationScreenState extends State<AuditLocationScreen>
  ...
  Future<void> _fetchLokasi() async {
    ...
    try {
      ...
      // NEW: simpan SEMUA baris audit per ref agar bisa dicocokkan dengan
      // id_schedule milik jadwal yang sedang aktif — skor hanya tampil
      // ketika berasal dari periode yang sama dengan periode/auditor/tipe
      // audit yang ditampilkan.
      final Map<String, List<Map<String, dynamic>>> auditListMap = {};
      for (final a in futures[0]) {
        final ref = a['id_ref'].toString();
        auditListMap.putIfAbsent(ref, () => []).add(a as Map<String, dynamic>);
      }
      ...
      // NEW: hanya jadwal yang periode-nya sedang berlangsung hari ini
      ...
      final items = rows.map<_LocationItem>((r) {
        ...
        String? scheduleAuditorImage;
        String? scheduleJenisAuditName;
        Map<String, dynamic>? scopedAudit;
        if (schedule != null) {
          ...
          // NEW: skor HANYA dari audit_result dengan id_schedule = jadwal aktif ini,
          // supaya skor selalu berasal dari periode yang sama dengan yang tampil.
          final idScheduleAktif = schedule['id_schedule']?.toString();
          for (final a in auditList) {
            if (a['id_schedule']?.toString() == idScheduleAktif) {
              scopedAudit = a;
              break;
            }
          }
        ...
        return _LocationItem(
          ...
          latestFinalScore: scopedAudit != null
              ? double.tryParse(scopedAudit['nilai_final']?.toString() ?? '')
              : null,
  ...
  double _nameFontSize(String name) {
    if (name.length > 32) return 12.5;
    if (name.length > 22) return 13.5;
    return 15;
  }
  ...
  Widget _buildCard(_LocationItem item) {
    ...
    final hasAuditor = item.scheduleAuditorName != null && item.scheduleAuditorName!.isNotEmpty;
    final hasSchedule = item.schedulePeriode != null && item.schedulePeriode!.isNotEmpty;
        ...
        child: Stack(
          ...
          children: [
            Column(
              children: [
                Padding(
                  padding: const EdgeInsets.fromLTRB(14, 14, 100, 14),
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Container(
                              ...
                              children: [
                                CircleAvatar(
                                  ...
                                  backgroundColor: hasPic
                                      ...
                                      : _C.textSub.withValues(alpha: 0.12),
                                  ...
                                  child: (item.picImage == null || item.picImage!.isEmpty)
                                      ? Icon(
                                          ...
                                          size: 12,
                                          color: hasPic ? _C.green : _C.textSub,
                                        )
                                      : null,
                                ),
                                const SizedBox(width: 6),
                                Expanded(
                                  child: Text(
                                    hasPic ? item.picName! : _t('No PIC assigned', 'Belum ada PIC', '暂无负责人'),
                                    style: GoogleFonts.poppins(
                                        fontSize: 11.5,
                                        fontWeight: FontWeight.w600,
                                        fontStyle: hasPic ? FontStyle.normal : FontStyle.italic,
                                        color: hasPic ? Colors.black87 : _C.textSub),
                                  ),
                                ),
                              ],
                            ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            hasAuditor
                                ...
                                : Row(
                                    children: [
                                      Icon(Icons.person_off_rounded, size: 13, color: _C.textSub),
                                      const SizedBox(width: 5),
                                      Expanded(
                    ...
                    ConstrainedBox(
                      ...
                      child: Container(
                        ...
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(Icons.fact_check_rounded, size: 10, color: _C.primary),
                            const SizedBox(width: 3),
                            Flexible(
                              child: Text(item.scheduleJenisAuditName!,
                                  style: GoogleFonts.poppins(
                                      fontSize: 9, fontWeight: FontWeight.w700, color: _C.primary),
                                  maxLines: 1, overflow: TextOverflow.ellipsis),
                            ),
                          ],
                        ),
          ...
          children: [
            Image.asset(
              ...
              errorBuilder: (_, __, ___) => Container(
                ...
                child: Icon(Icons.location_off_rounded, size: 56, color: _C.green.withValues(alpha: 0.4)),
              ),
            ),
            const SizedBox(height: 22),
            Text(
              isFiltering
                  ? _t('No matching locations', 'Lokasi Tidak Ditemukan', '未找到匹配位置')
                  : _t('No locations yet', 'Belum Ada Lokasi', '暂无位置'),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Icon(Icons.refresh_rounded, size: 15, color: _C.green),
                      const SizedBox(width: 6),
              ...
              children: [
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      focusedBorder: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(30),
                          borderSide: const BorderSide(color: _C.textMain, width: 1.5)),
                    ),
                  ),
                ),
                const SizedBox(width: 8),
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: _filter != null ? _C.textMain : _C.surface,
                      borderRadius: BorderRadius.circular(12),
                      border: Border.all(color: _filter != null ? _C.textMain : _C.divider),
                    ),
                    child: Icon(
                      Icons.filter_list_rounded,
                      color: _filter != null ? Colors.white : _C.textMain,
                      size: 20,
                    ),
                  ),
                ),
...
class _LocationDetailSheetState extends State<_LocationDetailSheet> {
  ...
  // NEW: rata-rata skor — nilai_audit jika 100, nilai_final jika di bawah 100
  // (fallback ke nilai_audit bila nilai_final belum ada), selaras dengan
  // tampilan per baris di Audit History.
  double? _computeAverageScore(List<Map<String, dynamic>> history) {
    final List<double> values = [];
    for (final row in history) {
      final raw = double.tryParse(row['nilai_audit']?.toString() ?? '');
      final fin = double.tryParse(row['nilai_final']?.toString() ?? '');
      if (raw == null) continue;
      values.add(raw >= 100 ? raw : (fin ?? raw));
    }
  ...
  Future<void> _fetchHistory() async {
    try {
      final rows = await _supabase
          ...
          .limit(20);
      if (mounted) {
        final historyList = List<Map<String, dynamic>>.from(rows);
        setState(() {
          _history = historyList;
          _avgScore = _computeAverageScore(historyList);
            ...
            children: [
              ...
              if (_avgScore != null) ...[
                ...
                Column(
                  crossAxisAlignment: CrossAxisAlignment.end,
                  children: [
                    Text(_t('Avg Score', 'Skor Rata-rata', '平均分'),
                        style: GoogleFonts.poppins(
                            fontSize: 9, fontWeight: FontWeight.w700, color: _scoreColor(_avgScore))),
                    const SizedBox(height: 4),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                      decoration: BoxDecoration(
                        color: _scoreColor(_avgScore).withValues(alpha: 0.12),
                        borderRadius: BorderRadius.circular(10),
                        border: Border.all(color: _scoreColor(_avgScore).withValues(alpha: 0.4)),
                      ),
                      child: Text('${_avgScore!.toStringAsFixed(0)}%',
                          style: GoogleFonts.poppins(
                              fontSize: 15, fontWeight: FontWeight.w800, color: _scoreColor(_avgScore))),
                    ),
                  ],
                ),
              ],
