...
class AdminSectionCameraWarmupService {
  AdminSectionCameraWarmupService._();
  static final AdminSectionCameraWarmupService instance =
      AdminSectionCameraWarmupService._();
  ...
  Future<void> warmUp() async {
    ...
    try {
      ...
      debugPrint('Error warming up Admin Section camera: $e');
    } finally {
      isWarming = false;
    }
  }
...
class AdminSectionCameraScreen extends StatefulWidget {
  final String lang;

  const AdminSectionCameraScreen({
    super.key,
    this.lang = 'ID',
  });

  @override
  State<AdminSectionCameraScreen> createState() =>
      _AdminSectionCameraScreenState();
}

class _AdminSectionCameraScreenState extends State<AdminSectionCameraScreen>
  ...
  Future<void> _initCamera() async {
    final warm = AdminSectionCameraWarmupService.instance;
    ...
    try {
      ...
      debugPrint('Error initializing Admin Section camera: $e');
    }
  }
  ...
  Future<void> _setCamera(int index) async {
    ...
    try {
      ...
      debugPrint(
          'Error setting Admin Section camera: ${e.code}\n${e.description}');
    }
  }
  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      debugPrint(
          'Error taking Admin Section picture: ${e.code}\n${e.description}');
      if (mounted) {
  ...
  Future<void> _pickFromGallery() async {
    try {
      ...
      debugPrint('Error picking Admin Section image: $e');
    }
  }
        ...
        child: TweenAnimationBuilder<double>(
          ...
          child: Column(
            ...
            children: [
              ...
              _AdminSectionCameraPulsingText(text: _loadingText, color: _kAccent),
            ],
          ),
        ),
      ),
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 10),
                      _AdminSectionCameraFlashButton(
...
class _AdminSectionCameraIconButton extends StatelessWidget {
  final IconData icon;
  final VoidCallback onTap;
  final double size;

  const _AdminSectionCameraIconButton({
    required this.icon,
    required this.onTap,
    this.size = 44,
  });
...
class _AdminSectionCameraFlashButton extends StatelessWidget {
  final bool supported;
  final bool enabled;
  final VoidCallback onTap;

  const _AdminSectionCameraFlashButton({
...
class _AdminSectionCameraPulsingText extends StatefulWidget {
  final String text;
  final Color color;

  const _AdminSectionCameraPulsingText({required this.text, required this.color});

  @override
  State<_AdminSectionCameraPulsingText> createState() =>
      _AdminSectionCameraPulsingTextState();
}

class _AdminSectionCameraPulsingTextState
    extends State<_AdminSectionCameraPulsingText>
