...
/// Dialog Edit Section — dipisah dari AdminAddSectionDialog.
/// Panggil lewat `showDialog(builder: (_) => AdminEditSectionDialog(...))`.
class AdminEditSectionDialog extends StatefulWidget {
  final String lang;
  ...
  @override
  State<AdminEditSectionDialog> createState() => _AdminEditSectionDialogState();
}

class _AdminEditSectionDialogState extends State<AdminEditSectionDialog> {
  ...
  void initState() {
    ...
    _namaCtrl = TextEditingController(text: existing['nama_section_id']?.toString() ?? '');
    _descCtrl = TextEditingController(text: _localizedDesc(existing));
    _gambarUrl = existing['gambar_section'] as String?;
    _selLokasi = existing['id_lokasi']?.toString();
    _selUnit = existing['id_unit']?.toString();
    _selSubunit = existing['id_subunit']?.toString();
    _selArea = existing['id_area']?.toString();
  ...
  // --------------------------------------------------------------------
  // Foto Section: kamera instan + fullscreen viewer
  // Tap gambar (saat sudah ada preview) -> fullscreen viewer.
  // Tap badge kamera pojok -> ganti foto.
  // --------------------------------------------------------------------
  ...
  Future<void> _onTapPhoto() async {
    ...
    try {
      ...
      final fileName = '${widget.existing['id_section']?.toString() ?? 'section'}-${DateTime.now().millisecondsSinceEpoch}.$safeExt';
  ...
  // --------------------------------------------------------------------
  // Picker Lokasi/Unit/Sub-Unit/Area
  // --------------------------------------------------------------------
  ...
  Future<void> _handleSave() async {
    ...
    try {
      final isDup = widget.sections.any((s) =>
          ...
          s['id_section'] != widget.existing['id_section']);
      if (isDup) {
      ...
      await _supabase.from('section').update(data).eq('id_section', widget.existing['id_section']);

      if (mounted) Navigator.pop(context);
      await widget.onSaved();
      widget.showResultPopup(
        isSuccess: true,
        titleEn: 'Section Updated!',
        titleId: 'Section Diperbarui!',
        titleZh: '部门已更新！',
        msgEn: 'Section has been updated successfully.',
        msgId: 'Section berhasil diperbarui.',
        msgZh: '部门已成功更新。',
      );
    } catch (e) {
      debugPrint('Error update section: $e');
                          ...
                          : () async {
                              ...
                              final result = await showDialog<Map<String, dynamic>>(
                                ...
                                builder: (dCtx) => _SectionPicPickerDialog(
                                  ...
                                  idCol: loc['idCol']!,
                                  locId: loc['id']!,
                                  excludeSectionId: widget.existing['id_section']?.toString(),
...
// --------------------------------------------------------------------
// Popup pilih PIC Section + filter Jabatan
// --------------------------------------------------------------------
