...
class AdminLocationCameraWarmupService {
  AdminLocationCameraWarmupService._();
  static final AdminLocationCameraWarmupService instance =
      AdminLocationCameraWarmupService._();
  ...
  Future<void> warmUp() async {
    ...
    try {
      ...
      debugPrint('Error warming up Admin Location camera: $e');
    } finally {
      isWarming = false;
    }
  }
...
class AdminLocationCameraScreen extends StatefulWidget {
  final String lang;

  const AdminLocationCameraScreen({
    super.key,
    this.lang = 'ID',
  });

  @override
  State<AdminLocationCameraScreen> createState() =>
      _AdminLocationCameraScreenState();
}

class _AdminLocationCameraScreenState extends State<AdminLocationCameraScreen>
  ...
  Future<void> _initCamera() async {
    final warm = AdminLocationCameraWarmupService.instance;
    ...
    try {
      ...
      debugPrint('Error initializing Admin Location camera: $e');
    }
  }
  ...
  Future<void> _setCamera(int index) async {
    ...
    try {
      ...
      debugPrint(
          'Error setting Admin Location camera: ${e.code}\n${e.description}');
    }
  }
  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      debugPrint(
          'Error taking Admin Location picture: ${e.code}\n${e.description}');
      if (mounted) {
  ...
  Future<void> _pickFromGallery() async {
    try {
      ...
      debugPrint('Error picking Admin Location image: $e');
    }
  }
        ...
        child: TweenAnimationBuilder<double>(
          ...
          child: Column(
            ...
            children: [
              ...
              _AdminLocationCameraPulsingText(text: _loadingText, color: _kAccent),
            ],
          ),
        ),
      ),
            ...
            Positioned(
              ...
              child: SafeArea(
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      _AdminLocationCameraIconButton(
                      ...
                      const SizedBox(width: 10),
                      _AdminLocationCameraFlashButton(
...
class _AdminLocationCameraIconButton extends StatelessWidget {
  final IconData icon;
  final VoidCallback onTap;
  final double size;

  const _AdminLocationCameraIconButton({
    required this.icon,
    required this.onTap,
    this.size = 44,
  });
...
class _AdminLocationCameraFlashButton extends StatelessWidget {
  final bool supported;
  final bool enabled;
  final VoidCallback onTap;

  const _AdminLocationCameraFlashButton({
...
class _AdminLocationCameraPulsingText extends StatefulWidget {
  final String text;
  final Color color;

  const _AdminLocationCameraPulsingText({required this.text, required this.color});

  @override
  State<_AdminLocationCameraPulsingText> createState() =>
      _AdminLocationCameraPulsingTextState();
}

class _AdminLocationCameraPulsingTextState
    extends State<_AdminLocationCameraPulsingText>
