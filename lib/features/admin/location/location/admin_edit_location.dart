...
class AdminEditLocationDialog extends StatefulWidget {
  final String lang;
  final Map<String, dynamic> existing;
  final Future<void> Function() onSaved;

  const AdminEditLocationDialog({
    super.key,
    required this.lang,
  ...
  @override
  State<AdminEditLocationDialog> createState() => _AdminEditLocationDialogState();
}

class _AdminEditLocationDialogState extends State<AdminEditLocationDialog> {
  ...
  String get _localizedDesc {
    ...
    if (widget.lang == 'EN') {
      return (e['deskripsi_lokasi_en'] ?? e['deskripsi_lokasi'] ?? '').toString();
    }
    if (widget.lang == 'ZH') {
      return (e['deskripsi_lokasi_zh'] ?? e['deskripsi_lokasi'] ?? '').toString();
    }
    return (e['deskripsi_lokasi'] ?? '').toString();
  }
  ...
  void initState() {
    ...
    _namaCtrl = TextEditingController(text: existing['nama_lokasi']?.toString() ?? '');
    _descCtrl = TextEditingController(text: _localizedDesc);
    _gambarUrl = existing['gambar_lokasi'] as String?;
  ...
  Future<void> _processPickedImage(XFile picked) async {
    ...
    try {
      ...
      final fileName = '${widget.existing['id_lokasi']?.toString() ?? 'lokasi'}-${DateTime.now().millisecondsSinceEpoch}.$safeExt';
  ...
  Future<void> _handleSave() async {
    ...
    try {
      await _supabase.from('lokasi').update(data).eq('id_lokasi', widget.existing['id_lokasi']);
          ...
          children: [
            Container(
              ...
              child: Row(children: [
                ...
                Expanded(
                  child: Text(
                    _t('Edit Location', 'Edit Lokasi', '编辑位置'),
