...
class AdminAreaCameraWarmupService {
  AdminAreaCameraWarmupService._();
  static final AdminAreaCameraWarmupService instance =
      AdminAreaCameraWarmupService._();
  ...
  Future<void> warmUp() async {
    ...
    try {
      ...
      debugPrint('Error warming up Admin Area camera: $e');
    } finally {
      isWarming = false;
    }
  }
...
class AdminAreaCameraScreen extends StatefulWidget {
  final String lang;

  const AdminAreaCameraScreen({
    super.key,
    this.lang = 'ID',
  });

  @override
  State<AdminAreaCameraScreen> createState() =>
      _AdminAreaCameraScreenState();
}

class _AdminAreaCameraScreenState extends State<AdminAreaCameraScreen>
  ...
  static const Color _kAccent = Color(0xFFF472B6);
  ...
  Future<void> _initCamera() async {
    final warm = AdminAreaCameraWarmupService.instance;
    ...
    try {
      ...
      debugPrint('Error initializing Admin Area camera: $e');
    }
  }
  ...
  Future<void> _setCamera(int index) async {
    ...
    try {
      ...
      debugPrint(
          'Error setting Admin Area camera: ${e.code}\n${e.description}');
    }
  }
  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      debugPrint(
          'Error taking Admin Area picture: ${e.code}\n${e.description}');
      if (mounted) {
  ...
  Future<void> _pickFromGallery() async {
    try {
      ...
      debugPrint('Error picking Admin Area image: $e');
    }
  }
        ...
        child: TweenAnimationBuilder<double>(
          ...
          child: Column(
            ...
            children: [
              ...
              _AdminAreaCameraPulsingText(text: _loadingText, color: _kAccent),
            ],
          ),
        ),
      ),
...
class _AdminAreaCameraIconButton extends StatelessWidget {
  final IconData icon;
  final VoidCallback onTap;
  final double size;

  const _AdminAreaCameraIconButton({
    required this.icon,
    required this.onTap,
    this.size = 44,
  });
...
class _AdminAreaCameraFlashButton extends StatelessWidget {
  final bool supported;
  final bool enabled;
  final VoidCallback onTap;

  const _AdminAreaCameraFlashButton({
...
class _AdminAreaCameraPulsingText extends StatefulWidget {
  final String text;
  final Color color;

  const _AdminAreaCameraPulsingText({required this.text, required this.color});

  @override
  State<_AdminAreaCameraPulsingText> createState() =>
      _AdminAreaCameraPulsingTextState();
}

class _AdminAreaCameraPulsingTextState
    extends State<_AdminAreaCameraPulsingText>
