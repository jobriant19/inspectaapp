...
class AdminEditAreaDialog extends StatefulWidget {
  final String lang;
  final Map<String, dynamic> existing;
  final List<Map<String, dynamic>> areas;
  final Future<void> Function() onSaved;

  const AdminEditAreaDialog({
    super.key,
    required this.lang,
  ...
  @override
  State<AdminEditAreaDialog> createState() => _AdminEditAreaDialogState();
}

class _AdminEditAreaDialogState extends State<AdminEditAreaDialog> {
  ...
  String _localizedDesc(Map<String, dynamic> a) {
    if (widget.lang == 'EN') return (a['deskripsi_area_en'] ?? a['deskripsi_area'] ?? '').toString();
    if (widget.lang == 'ZH') return (a['deskripsi_area_zh'] ?? a['deskripsi_area'] ?? '').toString();
    return (a['deskripsi_area'] ?? '').toString();
  }
  ...
  void initState() {
    ...
    _namaCtrl = TextEditingController(text: existing['nama_area']?.toString() ?? '');
    _descCtrl = TextEditingController(text: _localizedDesc(existing));
    _gambarUrl = existing['gambar_area'] as String?;
    _selSubunitId = existing['id_subunit']?.toString();
    _selSubunitName = existing['subunit']?['nama_subunit'] as String?;
    _derivedUnitId = existing['id_unit']?.toString();
    _derivedLokasiId = existing['id_lokasi']?.toString();
  ...
  Future<void> _onTapPhoto() async {
    ...
    try {
      ...
      final fileName = '${widget.existing['id_area']?.toString() ?? 'area'}-${DateTime.now().millisecondsSinceEpoch}.$safeExt';
  ...
  Future<void> _handleSave() async {
    ...
    if (_selSubunitId == null) {
      missing.add(MissingFieldItem(icon: Icons.layers_rounded, label: 'Subunit'));
    }
    if (_selPicId == null) {
    ...
    final isDup = widget.areas.any((a) =>
        (a['nama_area']?.toString().trim().toLowerCase() ?? '') == text.toLowerCase() &&
        a['id_area'] != widget.existing['id_area']);
    if (isDup) {
    ...
    try {
      await _supabase.from('area').update(data).eq('id_area', widget.existing['id_area']);
      if (mounted) Navigator.pop(context);
      await widget.onSaved();
    } catch (e) {
      debugPrint('Error update area: $e');
                          ...
                          : () async {
                              final result = await showDialog<Map<String, dynamic>>(
                                ...
                                builder: (dCtx) => _AreaPicPickerDialog(
                                  ...
                                  excludeAreaId: widget.existing['id_area']?.toString(),
