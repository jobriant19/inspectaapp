...
class AdminEditUnitDialog extends StatefulWidget {
  final String lang;
  final Map<String, dynamic> existing;
  final List<Map<String, dynamic>> lokasiList;
  final Future<void> Function() onSaved;

  const AdminEditUnitDialog({
    super.key,
    required this.lang,
  ...
  @override
  State<AdminEditUnitDialog> createState() => _AdminEditUnitDialogState();
}

class _AdminEditUnitDialogState extends State<AdminEditUnitDialog> {
  ...
  String _localizedDesc(Map<String, dynamic> u) {
    switch (widget.lang) {
      case 'EN':
        return (u['deskripsi_unit_en'] ?? u['deskripsi_unit'] ?? '').toString();
      case 'ZH':
        return (u['deskripsi_unit_zh'] ?? u['deskripsi_unit'] ?? '').toString();
      default:
        return (u['deskripsi_unit'] ?? '').toString();
    }
  }
  ...
  void initState() {
    ...
    _namaCtrl = TextEditingController(text: existing['nama_unit']?.toString() ?? '');
    _descCtrl = TextEditingController(text: _localizedDesc(existing));
    _gambarUrl = existing['gambar_unit'] as String?;
    _selLokasi = existing['id_lokasi']?.toString();
    _selLokasiName = existing['lokasi']?['nama_lokasi'] as String?;
  ...
  void _openFullscreenPreview() {
    ...
    Navigator.push(
      ...
      PageRouteBuilder(
        ...
        pageBuilder: (_, __, ___) => _UnitFullscreenImageViewer(
          imageUrl: url ?? '',
  ...
  Future<void> _onTapPhoto() async {
    final XFile? picked = await Navigator.of(context, rootNavigator: true).push<XFile?>(
      MaterialPageRoute(builder: (_) => AdminUnitCameraScreen(lang: widget.lang)),
    );
    ...
    try {
      ...
      final fileName = '${widget.existing['id_unit']?.toString() ?? 'unit'}-${DateTime.now().millisecondsSinceEpoch}.$safeExt';
  ...
  Widget _buildLocationPickerField({
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            const Icon(Icons.location_city_rounded, size: 14, color: _C.green),
            ...
            child: Row(
              children: [
                Icon(Icons.location_city_rounded,
                    size: 16, color: hasValue ? _C.green : Colors.black26),
                const SizedBox(width: 10),
                Expanded(
                  child: Text(
                    hasValue ? selectedName : _t('Select Location', 'Pilih Lokasi', '选择位置'),
                    style: GoogleFonts.poppins(
                    ...
                    Expanded(
                      child: Text(
                        enabled
                            ...
                            : _t('Select Location first', 'Pilih Lokasi dahulu', '请先选择位置'),
  ...
  Future<void> _openPicPicker() async {
    if (_selLokasi == null) return;
    final result = await showDialog<Map<String, dynamic>>(
      context: context,
      barrierDismissible: true,
      builder: (dCtx) => _UnitPicPickerDialog(
        lang: widget.lang,
        locId: _selLokasi!,
        excludeUnitId: widget.existing['id_unit']?.toString(),
  ...
  Future<void> _handleSave() async {
    ...
    if (text.isEmpty) {
      missing.add(MissingFieldItem(icon: Icons.business_rounded, label: _t('Unit Name', 'Nama Unit', '单位名称')));
    }
    if (_selLokasi == null) {
      missing.add(MissingFieldItem(icon: Icons.location_city_rounded, label: _t('Location', 'Lokasi', '位置')));
    }
    if (_selPicId == null) {
    ...
    showDialog(
      ...
      builder: (ctx) => _TranslatingDialog(color: _C.primary, lang: widget.lang),
    );
    ...
    final data = {
      ...
      'gambar_unit': _gambarUrl,
      'id_lokasi': _selLokasi,
      'id_pic': _selPicId,
    };

    await _supabase.from('unit').update(data).eq('id_unit', widget.existing['id_unit']);
  ...
  Widget build(BuildContext context) {
    final hasLocation = _selLokasi != null;
            ...
            Container(
              ...
              child: Row(
                children: [
                  Container(
                    padding: const EdgeInsets.all(10),
                    decoration: BoxDecoration(color: _C.primary.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(12)),
                    child: const Icon(Icons.business_rounded, color: _C.primary, size: 22),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Text(
                      _t('Edit Unit', 'Edit Unit', '编辑单位'),
                      style: GoogleFonts.poppins(color: _C.primary, fontWeight: FontWeight.w700, fontSize: 16),
                    ),
                  ),
                  GestureDetector(
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 6),
                        Text(
                          _t('Unit Photo', 'Foto Unit', '单位照片'),
                          style: GoogleFonts.poppins(color: _C.primary, fontSize: 12, fontWeight: FontWeight.w600, letterSpacing: 0.3),
                        ),
                      ],
                    ),
                    const SizedBox(height: 10),
                    _buildUnitPhotoPicker(
                      imageUrl: _gambarUrl,
                      previewBytes: _previewBytes,
                    ...
                    Row(
                      children: [
                        const Icon(Icons.business_rounded, size: 14, color: _C.primary),
                        const SizedBox(width: 6),
                        Text(
                          _t('Unit Name', 'Nama Unit', '单位名称'),
                          style: GoogleFonts.poppins(color: _C.primary, fontSize: 12, fontWeight: FontWeight.w600, letterSpacing: 0.3),
                        ),
                        const SizedBox(width: 3),
                    ...
                    Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: _t('Unit Name', 'Nama Unit', '单位名称'),
                          hintStyle: GoogleFonts.poppins(color: Colors.black26, fontSize: 13),
                          prefixIcon: const Icon(Icons.business_rounded, color: Colors.black38, size: 18),
                    ...
                    Container(
                      ...
                      child: TextField(
                        ...
                        style: GoogleFonts.poppins(color: Colors.black, fontSize: 14, fontWeight: FontWeight.w600),
                        decoration: InputDecoration(
                          hintText: _t('Description', 'Deskripsi', '描述'),
                          hintStyle: GoogleFonts.poppins(color: Colors.black26, fontSize: 13),
                          border: InputBorder.none,
                          contentPadding: const EdgeInsets.symmetric(vertical: 14, horizontal: 16),
                        ),
                    ...
                    Text(
                      hasLocation
                          ...
                          : _t('Select a Location above first.',
                              'Pilih Lokasi di atas terlebih dahulu.', '请先选择上方的位置。'),
                    ...
                    _buildPicField(
                      enabled: hasLocation,
                      picData: _selPicData,
                      onTap: !hasLocation ? null : _openPicPicker,
                    ),
                    const SizedBox(height: 8),
                  ],
                ),
                ...
                children: [
                  Expanded(
                    child: OutlinedButton(
                      ...
                      child: Text(_t('Cancel', 'Batal', '取消'), style: GoogleFonts.poppins(fontWeight: FontWeight.w600)),
                    ),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
...
class _UnitFullscreenImageViewer extends StatelessWidget {
  final String imageUrl;
  final Uint8List? previewBytes;

  const _UnitFullscreenImageViewer({required this.imageUrl, this.previewBytes});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(_title, style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w700, color: const Color(0xFF1E293B)), textAlign: TextAlign.center),
            const SizedBox(height: 6),
            Text(_subtitle, style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w500, color: const Color(0xFF64748B), height: 1.4), textAlign: TextAlign.center),
            const SizedBox(height: 22),
            ClipRRect(
...
class _UnitLocationPickerDialogState extends State<_UnitLocationPickerDialog> {
  ...
  void _applyFilter() {
    ...
    setState(() {
      _filtered = q.isEmpty
          ? widget.items
          : widget.items.where((e) => (e['nama_lokasi']?.toString() ?? '').toLowerCase().contains(q)).toList();
      _currentPage = 1;
    });
  }
  ...
  Widget _buildLocationCard(Map<String, dynamic> item) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSel ? _C.green.withValues(alpha: 0.08) : Colors.white,
          borderRadius: BorderRadius.circular(14),
          border: Border.all(color: isSel ? _C.green : Colors.grey.shade200, width: isSel ? 1.5 : 1),
        ),
        child: Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: _C.green.withValues(alpha: 0.14), borderRadius: BorderRadius.circular(10)),
              child: const Icon(Icons.location_city_rounded, size: 18, color: _C.green),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Text(name,
            ...
            if (isSel)
              ...
              const Icon(Icons.chevron_right_rounded, color: _C.green, size: 20),
          ],
        ),
      ),
    );
          ...
          children: [
            Image.asset(
              ...
              errorBuilder: (_, __, ___) => Container(
                ...
                child: Icon(Icons.search_off_rounded, size: 34, color: _C.green.withValues(alpha: 0.5)),
              ),
            ),
            const SizedBox(height: 14),
            Text(
            ...
            Text(
              isSearching
                  ...
                  : _t('No locations available.', 'Tidak ada lokasi tersedia.', '没有可用位置。'),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      const Icon(Icons.refresh_rounded, size: 14, color: _C.green),
                      const SizedBox(width: 6),
                      Text(_t('Clear search', 'Hapus pencarian', '清除搜索'),
                          style: GoogleFonts.poppins(fontSize: 11.5, fontWeight: FontWeight.w700, color: _C.green)),
                    ],
                  ),
                ),
              ),
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      ...
      child: ConstrainedBox(
        constraints: BoxConstraints(maxHeight: MediaQuery.of(context).size.height * 0.75, maxWidth: 440),
        child: Container(
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.location_city_rounded, color: _C.green, size: 20),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Text(
                        _t('SELECT LOCATION', 'PILIH LOKASI', '选择位置'),
                        style: GoogleFonts.poppins(fontSize: 17, fontWeight: FontWeight.w800, color: _C.green),
                      ),
                    ),
                    GestureDetector(
                      onTap: () => Navigator.pop(context),
              ...
              Padding(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(30),
                    border: Border.all(color: _C.green.withValues(alpha: 0.3), width: 1.2),
                  ),
                  child: Row(
                    children: [
                      const Icon(Icons.search_rounded, size: 20, color: _C.green),
                      const SizedBox(width: 10),
                      Expanded(
                        child: TextField(
                      ...
                      if (_searchCtrl.text.isNotEmpty)
                        GestureDetector(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(color: _C.green.withValues(alpha: 0.15), shape: BoxShape.circle),
              ...
              Flexible(
                child: _filtered.isEmpty
                    ...
                    : ListView(
                        padding: const EdgeInsets.fromLTRB(20, 12, 20, 12),
                        shrinkWrap: true,
                        children: pageItems.map(_buildLocationCard).toList(),
                      ),
              ),
...
class _UnitPicPickerDialog extends StatefulWidget {
  final String lang;
  final String locId;
  final String? excludeUnitId;
  final String? selectedUserId;

  const _UnitPicPickerDialog({
    required this.lang,
    required this.locId,
    required this.excludeUnitId,
    required this.selectedUserId,
  });

  @override
  State<_UnitPicPickerDialog> createState() => _UnitPicPickerDialogState();
}

class _UnitPicPickerDialogState extends State<_UnitPicPickerDialog> {
  final _supabase = Supabase.instance.client;
  final TextEditingController _searchCtrl = TextEditingController();
  ...
  Future<Set<String>> _fetchExcludedPicIds() async {
    ...
    for (final t in ['lokasi', 'subunit', 'area', 'section']) {
      try {
    ...
    try {
      final res = await _supabase.from('unit').select('id_unit, id_pic').not('id_pic', 'is', null);
      for (final row in (res as List)) {
        if (widget.excludeUnitId != null && row['id_unit']?.toString() == widget.excludeUnitId) {
          continue;
        }
        final id = row['id_pic']?.toString();
        if (id != null) excluded.add(id);
      ...
      debugPrint('Error fetch excluded pic (unit): $e');
    }
    return excluded;
  }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      debugPrint('Error load unit PIC users: $e');
  ...
  Future<void> _openRoleFilter() async {
    final result = await showDialog<Map<String, dynamic>?>(
      context: context,
      barrierDismissible: true,
      builder: (ctx) => _UnitRoleFilterDialog(
        lang: widget.lang,
  ...
  Widget build(BuildContext context) {
    ...
    final endIdx = (startIdx + _perPage) > _filtered.length ? _filtered.length : startIdx + _perPage;
    final pageItems = _filtered.isEmpty ? <Map<String, dynamic>>[] : _filtered.sublist(startIdx, endIdx);
    final roleActive = _roleFilterId != null || _roleFilterIsVerificator;
            ...
            child: Row(children: [
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  height: 44,
                  decoration: BoxDecoration(
                    color: roleActive ? _C.primary : Colors.white,
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(color: roleActive ? _C.primary : _C.primary.withValues(alpha: 0.35), width: 1.3),
                  ),
                  child: Icon(Icons.filter_list_rounded, color: roleActive ? Colors.white : _C.primary, size: 20),
                ),
              ),
            ]),
          ),
...
class _UnitRoleFilterDialog extends StatefulWidget {
  final String lang;
  final int? selectedId;
  final bool isVerificatorSelected;
  const _UnitRoleFilterDialog({
    required this.lang,
    required this.selectedId,
  ...
  @override
  State<_UnitRoleFilterDialog> createState() => _UnitRoleFilterDialogState();
}

class _UnitRoleFilterDialogState extends State<_UnitRoleFilterDialog> {
  final _supabase = Supabase.instance.client;
  final TextEditingController _searchCtrl = TextEditingController();
                          ...
                          return InkWell(
                            ...
                            child: Container(
                              ...
                              child: Row(children: [
                                ...
                                Expanded(
                                  child: Text(nama,
                                      style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w700, color: isSelected ? color : _C.textMain)),
                                ),
