...
class AdminUnitCameraWarmupService {
  AdminUnitCameraWarmupService._();
  static final AdminUnitCameraWarmupService instance =
      AdminUnitCameraWarmupService._();
  ...
  Future<void> warmUp() async {
    ...
    try {
      ...
      debugPrint('Error warming up Admin Unit camera: $e');
    } finally {
      isWarming = false;
    }
  }
...
class AdminUnitCameraScreen extends StatefulWidget {
  final String lang;

  const AdminUnitCameraScreen({
    super.key,
    this.lang = 'ID',
  });

  @override
  State<AdminUnitCameraScreen> createState() =>
      _AdminUnitCameraScreenState();
}

class _AdminUnitCameraScreenState extends State<AdminUnitCameraScreen>
  ...
  Future<void> _initCamera() async {
    final warm = AdminUnitCameraWarmupService.instance;
    ...
    try {
      ...
      debugPrint('Error initializing Admin Unit camera: $e');
    }
  }
  ...
  Future<void> _setCamera(int index) async {
    ...
    try {
      ...
      debugPrint(
          'Error setting Admin Unit camera: ${e.code}\n${e.description}');
    }
  }
  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      debugPrint(
          'Error taking Admin Unit picture: ${e.code}\n${e.description}');
      if (mounted) {
  ...
  Future<void> _pickFromGallery() async {
    try {
      ...
      debugPrint('Error picking Admin Unit image: $e');
    }
  }
        ...
        child: TweenAnimationBuilder<double>(
          ...
          child: Column(
            ...
            children: [
              ...
              _AdminUnitCameraPulsingText(text: _loadingText, color: _kAccent),
            ],
          ),
        ),
      ),
...
class _AdminUnitCameraIconButton extends StatelessWidget {
  final IconData icon;
  final VoidCallback onTap;
  final double size;

  const _AdminUnitCameraIconButton({
    required this.icon,
    required this.onTap,
    this.size = 44,
  });
...
class _AdminUnitCameraFlashButton extends StatelessWidget {
  final bool supported;
  final bool enabled;
  final VoidCallback onTap;

  const _AdminUnitCameraFlashButton({
...
class _AdminUnitCameraPulsingText extends StatefulWidget {
  final String text;
  final Color color;

  const _AdminUnitCameraPulsingText({required this.text, required this.color});

  @override
  State<_AdminUnitCameraPulsingText> createState() =>
      _AdminUnitCameraPulsingTextState();
}

class _AdminUnitCameraPulsingTextState
    extends State<_AdminUnitCameraPulsingText>
