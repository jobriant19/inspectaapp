...
class AdminProfileCameraWarmupService {
  AdminProfileCameraWarmupService._();
  static final AdminProfileCameraWarmupService instance =
      AdminProfileCameraWarmupService._();
  ...
  Future<void> warmUp() async {
    ...
    try {
      ...
      debugPrint('Error warming up Admin Profile camera: $e');
    } finally {
      isWarming = false;
    }
  }
...
class AdminProfileCameraScreen extends StatefulWidget {
  final String lang;

  const AdminProfileCameraScreen({
    super.key,
    this.lang = 'ID',
  });

  @override
  State<AdminProfileCameraScreen> createState() =>
      _AdminProfileCameraScreenState();
}

class _AdminProfileCameraScreenState extends State<AdminProfileCameraScreen>
  ...
  static const Color _kAccent = Color.fromARGB(255, 29, 199, 97);
  ...
  Future<void> _initCamera() async {
    final warm = AdminProfileCameraWarmupService.instance;
    ...
    try {
      ...
      debugPrint('Error initializing Admin Profile camera: $e');
    }
  }
  ...
  Future<void> _setCamera(int index) async {
    ...
    try {
      ...
      debugPrint(
          'Error setting Admin Profile camera: ${e.code}\n${e.description}');
    }
  }
  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      debugPrint(
          'Error taking Admin Profile picture: ${e.code}\n${e.description}');
      if (mounted) {
  ...
  Future<void> _pickFromGallery() async {
    try {
      ...
      debugPrint('Error picking Admin Profile image: $e');
    }
  }
        ...
        child: TweenAnimationBuilder<double>(
          ...
          child: Column(
            ...
            children: [
              ...
              _AdminProfileCameraPulsingText(text: _loadingText, color: _kAccent),
            ],
          ),
        ),
      ),
            ...
            Positioned(
              ...
              child: SafeArea(
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      _AdminProfileCameraIconButton(
                        icon: Icons.arrow_back_ios_new_rounded,
                        onTap: () => Navigator.pop(context, null),
                        size: 52,
                      ),
                      const SizedBox(width: 10),
                      // LABEL: ADMIN PHOTO PROFILE
                      Expanded(
                        child: Container(
                      ...
                      const SizedBox(width: 10),
                      _AdminProfileCameraFlashButton(
...
class _AdminProfileCameraIconButton extends StatelessWidget {
  final IconData icon;
  final VoidCallback onTap;
  final double size;

  const _AdminProfileCameraIconButton({
    required this.icon,
    required this.onTap,
    this.size = 44,
  });
...
class _AdminProfileCameraFlashButton extends StatelessWidget {
  final bool supported;
  final bool enabled;
  final VoidCallback onTap;

  const _AdminProfileCameraFlashButton({
...
class _AdminProfileCameraPulsingText extends StatefulWidget {
  final String text;
  final Color color;

  const _AdminProfileCameraPulsingText({required this.text, required this.color});

  @override
  State<_AdminProfileCameraPulsingText> createState() =>
      _AdminProfileCameraPulsingTextState();
}

class _AdminProfileCameraPulsingTextState
    extends State<_AdminProfileCameraPulsingText>
