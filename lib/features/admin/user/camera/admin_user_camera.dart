...
class AdminUserPhotoCameraWarmupService {
  AdminUserPhotoCameraWarmupService._();
  static final AdminUserPhotoCameraWarmupService instance =
      AdminUserPhotoCameraWarmupService._();
  ...
  Future<void> warmUp() async {
    ...
    try {
      ...
      debugPrint('Error warming up Admin User Photo camera: $e');
    } finally {
      isWarming = false;
    }
  }
...
class AdminUserPhotoCameraScreen extends StatefulWidget {
  ...
  @override
  State<AdminUserPhotoCameraScreen> createState() =>
      _AdminUserPhotoCameraScreenState();
}

class _AdminUserPhotoCameraScreenState
    extends State<AdminUserPhotoCameraScreen> with WidgetsBindingObserver {
  ...
  static const Color _kAccent = Color(0xFF6366F1);
  ...
  String get _labelText {
    if (widget.isEdit) {
      return _lang == 'EN'
          ...
          : _lang == 'ZH'
              ? '编辑用户头像照片'
              : 'EDIT FOTO PROFIL PENGGUNA';
    }
    return _lang == 'EN'
        ? 'ADD USER PHOTO PROFILE'
        : _lang == 'ZH'
            ? '添加用户头像照片'
            : 'TAMBAH FOTO PROFIL PENGGUNA';
  }
  ...
  Future<void> _initCamera() async {
    final warm = AdminUserPhotoCameraWarmupService.instance;
    ...
    try {
      ...
      debugPrint('Error initializing Admin User Photo camera: $e');
    }
  }
  ...
  Future<void> _setCamera(int index) async {
    ...
    try {
      ...
      debugPrint('Error setting Admin User Photo camera: ${e.code}\n${e.description}');
    }
  }
  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      debugPrint('Error taking Admin User Photo picture: ${e.code}\n${e.description}');
      if (mounted) {
  ...
  Future<void> _pickFromGallery() async {
    try {
      ...
      debugPrint('Error picking Admin User Photo image: $e');
    }
  }
        ...
        child: TweenAnimationBuilder<double>(
          ...
          child: Column(
            ...
            children: [
              ...
              _AdminUserPhotoPulsingText(text: _loadingText, color: _kAccent),
            ],
          ),
        ),
      ),
            ...
            Positioned(
              ...
              child: SafeArea(
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      _AdminUserPhotoIconButton(
                        icon: Icons.arrow_back_ios_new_rounded,
                        onTap: () => Navigator.pop(context, null),
                        size: 52,
                      ),
                      const SizedBox(width: 10),
                      // LABEL: ADD / EDIT USER PHOTO PROFILE
                      Expanded(
                        child: Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              const Icon(Icons.account_circle_outlined, color: _kAccent, size: 20),
...
class _AdminUserPhotoIconButton extends StatelessWidget {
  final IconData icon;
  final VoidCallback onTap;
  final double size;

  const _AdminUserPhotoIconButton({
    required this.icon,
    required this.onTap,
    this.size = 44,
  });
...
class _AdminUserPhotoFlashButton extends StatelessWidget {
  final bool supported;
  final bool enabled;
  final VoidCallback onTap;

  const _AdminUserPhotoFlashButton({
...
class _AdminUserPhotoPulsingText extends StatefulWidget {
  final String text;
  final Color color;

  const _AdminUserPhotoPulsingText({required this.text, required this.color});

  @override
  State<_AdminUserPhotoPulsingText> createState() => _AdminUserPhotoPulsingTextState();
}

class _AdminUserPhotoPulsingTextState extends State<_AdminUserPhotoPulsingText> with SingleTickerProviderStateMixin {
