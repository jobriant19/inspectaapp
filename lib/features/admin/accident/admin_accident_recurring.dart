...
class AdminAccidentRecurringTab extends StatefulWidget {
  final String lang;

  const AdminAccidentRecurringTab({
    super.key,
    required this.lang,
  });

  @override
  State<AdminAccidentRecurringTab> createState() =>
      AdminAccidentRecurringTabState();
}

class AdminAccidentRecurringTabState
    extends State<AdminAccidentRecurringTab> {
  ...
  Future<List<Map<String, dynamic>>> _fetchRecurring() async {
    try {
      ...
      if (_recurringUserId != null) {
        q = q.eq('id_pelapor', _recurringUserId!);
      }
  ...
  Widget build(BuildContext context) {
    final fromLabel =
        DateFormat('MMM yyyy', _locale).format(_recurringFrom);
    final toLabel =
        DateFormat('MMM yyyy', _locale).format(_recurringTo);
        ...
        child: FutureBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (_, snap) {
            ...
            return ListView.separated(
              ...
              itemBuilder: (_, i) => _AdminRecurringCard(
                group: groups[i],
                lang:  widget.lang,
                onTap: () =>
                    _showRecurringDetail(context, groups[i]),
              ),
            );
          },
        ),
      ...
      child: Column(
        ...
        Container(
          ...
          child: Icon(Icons.repeat_rounded,
              size: 36, color: _C.primary.withValues(alpha: 0.5)),
        ),
        const SizedBox(height: 16),
        Text(
          _t(
            'Tidak ada laporan kecelakaan berulang.',
  ...
  void _showRecurringDetail(
    ...
    final reports   =
        group['reports'] as List<Map<String, dynamic>>;
...
class _AdminRecurringCard extends StatelessWidget {
  final Map<String, dynamic> group;
  final String               lang;
  final VoidCallback         onTap;

  const _AdminRecurringCard({
    required this.group,
    required this.lang,
    required this.onTap,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final severity =
        group['severityPattern'] as String? ?? topic;
    ...
    final totalLabel = lang == 'ID'
        ? 'Total'
        : lang == 'ZH'
            ? '总计'
            : 'Total';
    final sevLabel = lang == 'ID'
        ? 'Pola: $severity'
        : lang == 'ZH'
            ? '模式: $severity'
            : 'Pattern: $severity';
...
class _AdminReportCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (data['area'] != null) {
      location =
          (data['area'] as Map)['nama_area'] ?? '';
    } else if (data['subunit'] != null) {
      location =
          (data['subunit'] as Map)['nama_subunit'] ?? '';
    } else if (data['unit'] != null) {
      location =
          (data['unit'] as Map)['nama_unit'] ?? '';
    ...
    final statusColor =
        isSelesai ? const Color(0xFF16A34A) : const Color(0xFFDC2626);
    final statusBg =
        isSelesai ? const Color(0xFFF0FDF4) : const Color(0xFFFEF2F2);
                ...
                Row(
                  ...
                  if (tingkat.isNotEmpty)
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color:
                            sevColor.withValues(alpha: 0.10),
                        borderRadius:
                            BorderRadius.circular(6),
                        border: Border.all(
                            color: sevColor, width: 1),
                      ),
                      child: Text(
                        tingkat,
                        style: GoogleFonts.poppins(
